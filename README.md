...
The audience is a scientific one, and the em dash reads as editorial voice. There were 624 of
them across the project when the rule was set, put there one at a time by someone who writes
with them by habit, so it is a test rather than a style note: `src/typography.test.ts` fails on
any em dash in `src/`, `scripts/`, `docs/`, `reference/`, the README or the HTML. A style note
is an audit, performed once by whoever is looking and passing silently thereafter, which is the
same reasoning as C1-FX-11.

**En dashes are kept** and are deliberately not matched. They are the correct mark for a range
or a coupled pair, and the project uses them that way throughout: 1 to 1000 kDa, sections 3 to
14, IgM-PE. Removing those would cost precision rather than rhetoric.

Two strings changed that were specified verbatim elsewhere, and both need the same edit in
v0.6 so the specification and the tool do not disagree: C1-MW-07's conjugate option label, and
the retention badge.

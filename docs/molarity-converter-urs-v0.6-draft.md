...
| C1-UN-07 ✎ | The unrounded value shall be present in the structured object, and is the value against which an independent reimplementation is compared. **Where a computed quantity underflows the representable range and is reported as zero, the structured object shall mark it as underflowed.** A zero that is not the value is not a rounded value. | M |
